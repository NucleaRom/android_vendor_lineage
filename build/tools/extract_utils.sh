...
function setup_vendor() {
    ...
    if [ ! -d "$NR_ROOT" ]; then
        echo "\$NR_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$NR_ROOT/$OUTDIR" ]; then
        mkdir -p "$NR_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$NR_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$NR_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$NR_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$NR_ROOT"/vendor/nr/build/tools/smali/baksmali.jar
        export SMALIJAR="$NR_ROOT"/vendor/nr/build/tools/smali/smali.jar
    fi
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$NR_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$NR_ROOT"/vendor/nr/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$NR_ROOT"/"$OUTDIR"/radio
